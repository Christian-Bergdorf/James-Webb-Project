...
% ode45 function solves the differential equations of motion using inital
% condtions and intagrates over time untill burnout
...
% Max dynamic pressure and time of occurance solved for using values from for loop
[maxQ,imax] = max(q); 
tQ = t1(imax); 
vQ = v(imax); 
hQ = h(imax); 
...
% equations of motion for gravty turn path
% inital vector is defined
dzdt = zeros(6,1);
v     = z(1); 
gamma = z(2);
x     = z(3);
...
% Density vs altitude, gravational change with altitude and drag at given
% velocity calculated for later steps
g = g0/(1 + h/Re)^2; 
rho = rho0*exp(-h/ho); 
D = 0.5*rho*v^2*A*CD; 
% First if statement for vertical flight using differential equations of motion up
% untill altitude is above 130m. Then gravity assist begins in else
% statement
if h <= hturn
   gamma_dot = 0;
   v_dot  = T/m - D/m - g;
...
% fprintf('\n Maximum Dynamic Pressure  = %f kPa ',maxQ*9.869e-6*101.325)
% fprintf('\n Time of Maximum Dynamic Pressure = %f min ',tQ/60)
% plot of Altitude vs Downrange Distance
figure (1)
plot(x,h)
...
ylabel('Altitude (km)')
grid
% plot of Dynamic Pressure vs Time
% figure (2)
% plot(t1, q*9.869e-6*101.325)
